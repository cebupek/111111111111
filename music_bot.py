...
Установка: pip install python-telegram-bot==20.8 fastapi uvicorn httpx
...
# ============================================================
#  Telegram WebApp / проверка подписи
# ============================================================
...
def get_mod_from_request(request: Request) -> dict | None:
    ...
    if user.get("id") not in MOD_IDS: return None
    return user
...
async def notify_moderators(text: str):
    ...
    for mod_id in MOD_IDS:
        ...
        except Exception as e: log.warning(f"Не удалось уведомить {mod_id}: {e}")

async def on_new_song(data: dict):
    artist  = data.get("artist") or "❓ Неизвестен"
    title   = data.get("title")  or "❓ Неизвестно"
    tag     = "\n⚠️ <b>Трек без подписи</b>" if not data.get("artist") or not data.get("title") else ""
...
async def do_action(request: Request):
    ...
    try:
        if action == "approve":
            ...
            mark_handled(key, mod_name); add_history(item_type, item_id, "approve", mod_name)
        elif action == "reject":
            ...
            mark_handled(key, mod_name); add_history(item_type, item_id, "reject", mod_name, reason_text)
        elif action == "sign":
            artist = data.get("artist", "").strip(); title  = data.get("title", "").strip()
    ...
    except Exception as e: log.error(f"action error {item_type} {item_id}: {e}"); return JSONResponse({"error": str(e)}, status_code=500)
...
async def site_webhook(request: Request):
    if request.headers.get("X-Secret") != SECRET: return JSONResponse({"error": "forbidden"}, status_code=403)
    data  = await request.json(); event = data.get("type")
    try:
    ...
    except Exception as e: log.error(e); return JSONResponse({"error": str(e)}, status_code=500)
    return JSONResponse({"ok": True})
...
async def check_pending():
    log.info(f"Polling каждые {INTERVAL} сек."); await asyncio.sleep(5)
    prev_total = -1
    while True:
        try:
            songs  = (await api("GET", "/api/bot/pending/songs")).get("data", [])
        ...
        except Exception as e: log.error(f"Polling error: {e}")
...
async def main():
    ...
    config = uvicorn.Config(app_web, host="0.0.0.0", port=PORT, log_level="warning")
