...
<b>✅ Принимаем:</b> оригинальные треки, существующие песни, 1–10 минут
<b>❌ Отклоняем:</b> голосовые, длинные, политические, провокационные
<b>💡 Совет:</b> если нет подписи, проверь в интернете и подпиши ✍️"""

REJECT_REASONS = {
    "song":[{"label":"🔇 Не трек","code":"not_a_track"},{"label":"🤬 Неприемлемый","code":"bad_content"},{"label":"⚡ Политический","code":"political"},{"label":"📋 Авторские права","code":"copyright"},{"label":"⏱ Некорректная длина","code":"bad_length"},{"label":"🔁 Дубликат","code":"duplicate"},{"label":"✏️ Другая причина","code":"other"}],
    "name":[{"label":"🤬 Оскорбительное название","code":"offensive"},{"label":"⚡ Политический","code":"political"},{"label":"📢 Спам / реклама","code":"spam"},{"label":"✏️ Другая причина","code":"other"}],
    "cover":[{"label":"🔞 Неприемлемое изображение","code":"nsfw"},{"label":"©️ Чужое изображение","code":"copyright"},{"label":"🖼 Плохое качество","code":"bad_quality"},{"label":"✏️ Другая причина","code":"other"}]
}
REASON_TEXT = {
    "not_a_track":"не является треком","bad_content":"неприемлемый контент","political":"политический подтекст",
    "copyright":"нарушение авторских прав","bad_length":"некорректная длина","duplicate":"дубликат",
    "offensive":"оскорбительное название","spam":"спам / реклама","nsfw":"неприемлемое изображение",
    "bad_quality":"плохое качество","other":"другая причина"
}
...
def add_history(item_type, item_id, action, mod_name, reason=""):
    history.appendleft({"type":item_type,"id":item_id,"action":action,"mod":mod_name,"reason":reason,"time":datetime.now().strftime("%d.%m %H:%M")})
...
def get_mod_from_request(request: Request) -> dict | None:
    ...
    if not user or user.get("id") not in MOD_IDS: return None
    return user
...
async def notify_moderators(text:str):
    ...
    for mod_id in MOD_IDS:
        try: await bot.send_message(mod_id,text,parse_mode="HTML",reply_markup=keyboard)
        except Exception as e: log.warning(f"Не удалось уведомить модератора {mod_id}: {e}")
...
async def cmd_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🎛 Открыть панель",web_app=WebAppInfo(url=f"{APP_URL}/app"))]])
    )
...
async def cmd_panel(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id not in MOD_IDS:
        await update.message.reply_text("⛔ Нет доступа."); return
    await update.message.reply_text("🎛 Открой панель модерации:",reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🎛 Панель модерации",web_app=WebAppInfo(url=f"{APP_URL}/app"))]]))

# ============================================================
@app_web.get("/app")
async def serve_app(): return FileResponse(Path(__file__).parent / "app.html", media_type="text/html")
...
async def do_action(request: Request):
    ...
    if already and action != "sign": return JSONResponse({"error":"already_handled","by":already},status_code=409)
    try:
        if action=="approve":
            ...
            if not artist or not title: return JSONResponse({"error":"empty_fields"},status_code=400)
...
async def get_rules(request: Request): return JSONResponse({"rules":RULES_TEXT})
...
async def check_pending():
    ...
    while True:
        try:
            ...
            total = sum(pending_count.values())
            if prev_total>0 and total==0:
                for mod_id in MOD_IDS:
                    try: await bot.send_message(mod_id,"🎉 <b>Очередь пуста! Всё обработано.</b>",parse_mode="HTML")
                    except: pass
            prev_total = total
            for item in songs:
                if item["id"] not in sent_songs: sent_songs.add(item["id"]); await on_new_song(item)
            for item in names:
                if item["id"] not in sent_names: sent_names.add(item["id"]); await on_new_playlist_name(item)
            for item in covers:
                if item["id"] not in sent_covers: sent_covers.add(item["id"]); await on_new_cover(item)
        except Exception as e:
